...
Audit evidence is based on the current workspace and the commands recorded in the project history.
...
| R02 exact generator constants/logic | `generate_orders.py` | Exact generator validation | Reference constants/logic applied; 6000x13 output | PASS |
...
| R05 MAR missingness | `reports/return_risk_analysis.json` | Part 1 analysis | MAR; gap 16.77 pp | PASS |
...
| R10 RF grid/CV/test | `return_risk_report.json` | Part 1 training | CV 0.61797; test 0.61755 | PASS |
...
| R14 RF threshold metadata | `models/return_risk_metadata.json` | `pytest -q` | t*=0.47 | PASS |
...
The FAISS index is intentionally rebuilt locally from the committed knowledge base rather than storing a generated binary.